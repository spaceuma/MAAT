...
classdef catsHTM 
    ...
    methods (Static)
        ...
        function ID=search_htm_ind(FileName,VarName,Long,Lat,Radius,Catalog_Path)
...
%                    keyboard
            if (Check)
                DataHTM = HDF5.load_check(FileName,VarName,Catalog_Path);
%                 DataHTM = HDF5.load_check(strcat('C:\TransientDetection\catalogs\index\',FileName),VarName);
            else
    ...
    % search
    methods (Static)
        function [Cat,ColCell,ColUnits]=cone_search(CatName,RA,Dec,Radius,Catalog_Path,MagCol,MagnitudeThreshhold,varargin)
...
%             load(strcat('C:\TransientDetection\catalogs\index\',InPar.ColCellFile));
            ...
            ID     = catsHTM.search_htm_ind(IndexFileName,InPar.IndexVarName,RA,Dec,Radius,Catalog_Path);
            ...
            LoadBool = 0;
            nAttemp = 0;
            if contains(InPar.ColCellFile,'GAIA')
                    FolderPath = 'GAIA/DR2/';
            ...
            while ~LoadBool || nAttemp > 20
                try
                    for Iid=1:1:Nid
                        ...
                        if (Iid==1)
                            Cat = HDF5.load(FileName,DataName,Catalog_Path);
                            Cat(Cat(:,MagCol)>MagnitudeThreshhold | Cat(:,MagCol)==-999,:)=[];
                            %Cat = HDF5.load_cat(FileName,DataName,[MinDec, MaxDec],Ncol);

                            %Ncol = size(Cat,2);
                        else
                            buf = HDF5.load(FileName,DataName,Catalog_Path);
                            buf(buf(:,MagCol)>MagnitudeThreshhold | buf(:,MagCol)==-999,:)=[];
                            Cat = [Cat; buf];
                            %Cat = [Cat; HDF5.load_cat(FileName,DataName,[MinDec, MaxDec],Ncol)];
                        end
